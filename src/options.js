...
<a name="top"></a>
<h1>Better web <span id="myVersion" class="label" style="font-size:15px; color:#8ebf42;"></span></h1>
...
<fieldset>
  ...
  <button id="showPubKey">Server Public Key</button>

  <input type="file" style="display:none;" id="file-selector">    <button id="github">Import from Github</button>

  <button id="export">Export chatgpt Context</button>

   <button id="share">Export to Share With Teammate</button>
   <button id="publish">Export to Publish</button>
  <a href="https://github.com/ld32/betterWeb/tree/main" id='howToSetup' style="margin-right: 20px; margin-left: 20px;" target="_blank" rel="noopener noreferrer" class="btn btn-warning teaser">How to use?</a>
  <a href="https://chromewebstore.google.com/detail/my-prompts/ohmlhcmmjbponikechknmhedgojoaamp" style="margin-right: 20px;" target="_blank" rel="noopener noreferrer" class="btn btn-warning teaser">Please Rate Us</a>

   <a href="https://ld32.github.io/betterWeb/testPage/incidentList.html?view=my" style="margin-right: 20px;" target="_blank" rel="noopener noreferrer" class="btn btn-warning teaser">Testing page</a>
...
<fieldset>
  ...
  <a href="#rag-panel" style="margin-right: 20px;">Local AI database</a>
  ...
  <a href="#bookmark" style="margin-right: 0;">Bookmark</a>
</fieldset>
...
<!-- Second section: rag-panel.js mounts itself here. Empty in this template
     because that module owns the markup; it falls back to appending at the end
     of the page if this slot is ever removed. -->
<div id="rag-panel-slot"></div>
...
<a name="bookmark"></a>
<label for="explicit-label-name" style="font-size:20px;">Bookmark:</label>
<fieldset>
...
function checkForDuplicateConfigs(importedObj) {
  var config, duplicates = [], existingKeys = new Set();
  ...
  for (config of [ "configa", "configb", "configc", "configd" ]);
  if (importedObj && importedObj.urls) {
    var importedKeys = Object.keys(importedObj.urls);
    for (let i = 0; i < importedKeys.length; i++) {
      var key = importedObj.urls[importedKeys[i]][0];
      existingKeys.has(key) && duplicates.push(key), existingKeys.add(key);
    }
  }
  return duplicates;
}
...
document.getElementById("Config1").addEventListener("click", function(evt) {
  ...
  (evt.ctrlKey || evt.metaKey) && "s" === key && (evt.preventDefault(), key = document.getElementById("save")) && key.click();
  ...
  var obj = {
    ...
    fills: fillArray,
    bookmark: favorArray
  }, today = getDate().replace(/\s+/g, ""), obj = JSON.stringify(obj, null, 2).replace(/#/g, "&NPOUND"), obj = new Blob([ obj ], {
    type: "text/plain"
  });
  chrome.downloads.download({
    url: URL.createObjectURL(obj),
    filename: "Config." + configMap.get(configMap.get("current")).replace(/\s+/g, "-") + "." + today + ".json",
...
}), document.getElementById("publish").addEventListener("click", function(evt) {
  var myUrlArray = {};
  for (let i = 0; i < Object.keys(urlArray).length; i++) {
    var key = urlArray[i][0];
    let value = urlArray[i][1];
    "myFirstName" === key && (value = "David"), myUrlArray[i] = [ key, value ];
  }
  var obj = {
    urls: myUrlArray,
    keywords: keywordArray,
    ...
    sentences: {
      ...
      8: [ 1, "Thanks for hel" ],
      19: [ 1, "Here is correct one:" ],
      20: [ 1, "Good point." ],
      21: [ 1, "Does starfish scan standby folder?" ],
      22: [ 1, "200 is good number." ],
      23: [ 1, "Thanks agai" ],
      24: [ 1, '<span zeum4c1="' ],
      25: [ 4, "Thanks for the information. " ]
    },
    hides: hideArray,
    fills: fillArray
  }, today = getDate().replace(/\s+/g, ""), obj = JSON.stringify(obj, null, 2).replace(/#/g, "&NPOUND"), obj = new Blob([ obj ], {
  ...
  chrome.downloads.download({
    url: URL.createObjectURL(obj),
    filename: "Config.Publish." + configMap.get(configMap.get("current")).replace(/\s+/g, "-") + "." + today + ".json",
    conflictAction: "uniquify",
    saveAs: !0
  }, function(downloadId) {
    chrome.storage.local.set({
  ...
  var obj = {
    ...
    shortcuts: shortcutArray,
    bookmark: favorArray
  }, today = getDate().replace(/\s+/g, ""), obj = JSON.stringify(obj, null, 2), obj = new Blob([ obj ], {
    type: "text/plain"
  });
  chrome.downloads.download({
    url: URL.createObjectURL(obj),
    filename: "ChatContext." + today + ".json",
    conflictAction: "uniquify",
    saveAs: !0
  }, function(downloadId) {
    chrome.runtime.lastError;
  ...
  chrome.downloads.download({
    url: URL.createObjectURL(obj),
    filename: "ShareWithTeamMate." + today + ".json",
    conflictAction: "uniquify",
    saveAs: !0
  }, function(downloadId) {});
...
fileSelector.addEventListener("change", event => {
  var event = event.target.files, newData = "", reader = new FileReader();
  reader.onload = function(event) {
    event = event.target.result;
    if (0 !== (newData = (newData = (newData = (newData = (newData = event.replace(/&NPOUND/g, "#")).replace(/:\n\s*{ /g, ":{")).replace(/],\n\s{2}/g, "],")).replace(/},\n\n/g, "},")).replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "")).length) {
...
function getGitbubFileList() {
  ...
  apiRequest("GET", url, null, function(r) {
    apiRequest("GET", JSON.parse(r).commit.commit.tree.url + "?recursive=1", null, function(r) {
      ...
      for (i in cnt.innerHTML = "", jsonRsp.tree) jsonRsp.tree[i].path.endsWith(".json") && jsonRsp.tree[i].path.startsWith("Config") && ((li = document.createElement("li")).style.color = "red", 
...
function applyOptionsTheme() {
  chrome.storage.local.get([ "darkMode" ], function(result) {
    result = String(result && result.darkMode || "").trim().toLowerCase();
    document.documentElement.dataset.bwTheme = "yes" === result || "true" === result || "on" === result || "1" === result ? "dark" : "light";
  });
}
...
function toOriginPattern(token) {
  token = String(token || "").trim().replace(/^[a-z][a-z0-9+.-]*:\/\//i, "").split("/")[0].split(":")[0].toLowerCase();
  return /^[a-z0-9-]+(\.[a-z0-9-]+)*\.[a-z]{2,}$/.test(token) ? `*://*.${token}/*` : null;
}

function requestConfiguredSiteAccess() {
  const tokens = [], patterns = ([ "Config1", "Config2", "Config3", "Config4" ].forEach(name => {
    name = configMap.get(name);
    name && tokens.push.apply(tokens, String(name).trim().split(/\s+/));
  }), tokens.map(toOriginPattern).filter(Boolean));
  var origins = patterns.filter((p, i) => patterns.indexOf(p) === i), skipped = tokens.filter(t => t && !toOriginPattern(t));
  skipped.length, origins.length && chrome.permissions.request({
    origins: origins
  }, function(granted) {
    chrome.runtime.lastError ? tempAlert("Could not request access to the configured sites: " + chrome.runtime.lastError.message, 5e3) : (granted || tempAlert("Access was declined, so the extension will not run on the sites you added. Save again to be asked once more.", 6e3), 
    chrome.runtime.sendMessage({
      type: "sync-site-scripts"
    }, function() {
      chrome.runtime.lastError;
    }));
  });
...
function saveAll(closeIt) {
  ...
  configMap.set(configMap.get("current"), "")), requestConfiguredSiteAccess();
  let da = {}, index = 0;
  for (k of configMap.keys()) da[index] = [ k, configMap.get(k) ], index++;
  var key = "myConfigs", currentTime = JSON.stringify(da, null, 2), n1 = (chrome.storage.local.set({
    [key]: currentTime
  ...
  var currentTime = {}, currentTime = (currentTime[1] = urlArray, currentTime[2] = keywordArray, 
  currentTime[3] = controlArray, currentTime[4] = shortcutArray, currentTime[5] = sentenceArray, 
  currentTime[6] = hideArray, currentTime[7] = fillArray, currentTime[8] = favorArray, 
  JSON.stringify(currentTime, null, 2)), savedConfig = {
    urls: urlArray,
    keywords: keywordArray,
    controls: controlArray,
    shortcuts: shortcutArray,
  ...
  chrome.storage.local.set({
    betterWeb_user_config_json: JSON.stringify(savedConfig)
  }, function() {}), "Config1" === configMap.get("current") ? chrome.storage.local.set({
    configa: currentTime
...
function downloadAndParseGithub(url, fileName) {
  githubApi.path = fileName, githubApi.newFile = !1, apiRequest("GET", url, null, function(r) {
    cout(r);
    r = r.replace(/&NPOUND/g, "#").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, ""), 
    r = JSON.parse(r);
    if (cout(r), "base64" != r.encoding) cout("unknown encoding " + r.encoding); else {
      var decoded = decodeURIComponent(escape(window.atob(r.content))), obj = (decoded = (decoded = (decoded = decoded.replace(/:\n\s*{ /g, ":{")).replace(/],\n\s{2}/g, "],")).replace(/},\n\n/g, "},"), 
      JSON.parse(decoded.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "")));
...
document.getElementById("github").addEventListener("click", function(evt) {
  ...
  deleteAll();
}), applyOptionsTheme(), chrome.storage.onChanged.addListener(function(changes, area) {
  "local" === area && changes.darkMode && applyOptionsTheme();
}), document.getElementById("showPubKey")?.addEventListener("click", function() {
  chrome.storage.local.get([ "signingPublicKey" ], function(res) {
    var res = res.signingPublicKey;
    res ? (res = `EXTENSION_PUBLIC_KEY=${res} node RAGServer/server.js`, prompt("Server Public Key Command — copy this:", res)) : alert("No signing public key found. The extension will generate one on first use. Try reloading the extension.");
  });
});