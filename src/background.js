...
chrome.runtime.onInstalled.addListener(function(details) {
  ...
  if (!request) return !1;
  if ("query-phi3-stream" === request.type) return fetch("http://localhost:5000/chat", {
    method: "POST",
    headers: {
    ...
    if (res.body) {
      ...
      for (;;) {
        ...
        if (done) break;
        var line, done = decoder.decode(value, {
          stream: !0
        }), value = (buffer += done).split("\n");
        buffer = value.pop();
        for (line of value) if (line.trim()) try {
          var chunk = JSON.parse(line).response || "";
          sender.tab && void 0 !== sender.tab.id && chrome.tabs.sendMessage(sender.tab.id, {
      ...
      ok: !1,
      error: "No response body from chat server"
    });
  }).catch(err => {
    sender.tab && void 0 !== sender.tab.id && chrome.tabs.sendMessage(sender.tab.id, {
      type: "phi3-stream-chunk",
  ...
  if ("upload-conversation" === request.type) return fetch("http://localhost:5000/upload_only", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify(request.data || request.conversation || {})
  }).then(async res => {
    let data;
    try {
      data = await res.json();
      ...
      data = {
        error: "Got error from server (note added in background script)"
      };
    }
    res.ok ? sendResponse({
      ok: !0,
  ...
  if ("classify-conversation" === request.type) return payload = request.data || request.conversation || {}, 
  fetch("http://localhost:5000/classify_conversation", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    ...
    var m, referenceTickets;
    res.ok ? (m = `Service: ${(m = body && body.predicted_metadata || {}).service_type || "N/A"} | Ticket Type: ${m.ticket_type || "N/A"} | Request Type: ${m.request_type || "N/A"} | App/Hardware: ${m.app_hardware || "N/A"} | Assignment Group: ` + (m.assignment_group || "N/A"), 
    referenceTickets = Array.isArray(body && body.references_used) ? body.references_used : [], 
    sendResponse({
      ok: !0,
      classification: m,
      ...
      ok: !1,
      error: body.error || "Classification failed with HTTP " + res.status
    });
  }).catch(err => {
    sendResponse({
      ok: !1,
  ...
  if ("test-fetch" === request.type) return fetch("http://localhost:5000/chat").then(res => res.json()).then(data => {}).catch(err => {}), 
  !0;
    ...
    "hello" === request.greeting) sendResponse({
      ...
      sendResponse({
        ...
        if (request.greeting.startsWith("answerFromChatgpt")) sendResponse({
          ...
          var payload, arr = request.greeting.split(" ");
          ...
          }) : -1 !== request.greeting.indexOf("new tab") ? (payload = arr[1], chrome.tabs.query({
            url: payload + "/*"
          }, tabs => {
