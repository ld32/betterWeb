if (!window.__betterWebContentLoaded) {
  ...
  function ensureEditControlModeBadge() {
    var existing = document.getElementById("betterweb-edit-control-mode");
    return existing || ((existing = document.createElement("div")).id = "betterweb-edit-control-mode", 
    existing.style.position = "fixed", existing.style.left = "5px", existing.style.bottom = "5px", 
    existing.style.zIndex = "2147483646", existing.style.padding = "6px 10px", existing.style.borderRadius = "4px", 
    existing.style.background = "rgba(0, 0, 0, 0.8)", existing.style.color = "#fff", 
    existing.style.font = "8px/1.3 Calibri, Arial, Helvetica, sans-serif", existing.style.pointerEvents = "none", 
    existing.textContent = "Edit/Control mode: Control mode", document.body.appendChild(existing), 
    existing);
  }
  function updateEditControlMode(isEditing) {
    editingMode = !!isEditing;
    isEditing = ensureEditControlModeBadge();
    isEditing.textContent = editingMode ? "Edit mode" : "Control mode", isEditing.style.background = editingMode ? "rgba(26, 115, 232, 0.92)" : "rgba(0, 0, 0, 0.8)";
  }
  function setupEditControlModeStatus() {
    ensureEditControlModeBadge(), updateEditControlMode(!1);
  }
  ...
  function processData(ob) {
    ...
    listPagePaths = splitWords(urlMap.get("listPagePaths")), (detailPagePaths = splitWords(urlMap.get("detailPagePaths"))).some(path => currentURL.includes(path))) {
      useBackgroundAPI = !1, aiProvider = urlMap.get("apiProvider") || "ollama", 
      ollamaUrl = urlMap.get("ollamaUrl") || "", ollamaModel = urlMap.get("ollamaModel") || "", 
      openaiApiKey = urlMap.get("openaiApiKey") || "", "myFirstName" === (myFirstName = void 0 !== myFirstName && 0 != myFirstName.length ? myFirstName : "myFirstName") && -1 != currentURL.indexOf("service-now") && alert("Please set up your name in option page."), 
      obj = ob[2];
      ...
      for (let i = 0; i < keys.length; i++) if (-1 == obj[keys[i]][0].indexOf("#")) {
        ...
        "Save selection to profile" === obj[keys[i]][1] && chrome.runtime.sendMessage({
          ...
          chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
            if (void 0 !== request.greeting && request.greeting.startsWith("SaveToProfile:")) {
              chrome.storage.local.set({
      ...
      if (-1 != currentURL.indexOf("office.com/calendar")) (leftLineParts = new MutationObserver(function(mutations) {
        ...
        if (-1 != currentURL.indexOf("outlook.office.com")) window.onfocus = function() {
          ...
          document.addEventListener("click", e => {
            ...
            changeBackground("white")) : -1 != currentURL.indexOf("incident_do") && "" == $('[name*="assigned_to"], [id*="assigned_to"]').val() || -1 != currentURL.indexOf("incident_task") && "" === $('input[name="sys_display.u_incident_task.assigned_to"]').val() || -1 != currentURL.indexOf("sc_task") && "" === $('input[name="sys_display.sc_task.assigned_to"]').val() ? (changeBackground(urlMap.get("newTicketColor")), 
  ...
  function makeDescriptionReadOnly() {
    ...
    for (key in chrome.storage.local.get([ currentID + "color" ], function(result) {
      ...
      assignedTo = $('[name*="assigned_to"], [id*="assigned_to"]').val(), "green" === result && assignedTo && -1 != assignedTo.indexOf(myFirstName) || ($('textarea[name="incident.short_description"]').css("background-color", result), 
  ...
  function hintWindow(msg) {
    ...
    el.setAttribute("id", "tempAlertDiv1"), el.style.cssText = `
    position: fixed;
    background: black;
    color: white;
    padding: 10px;
...
`, el.textContent = msg;
  ...
  function tmpAlert(msg, duration = 1e3, renderHtml = !1) {
    ...
    renderHtml ? existingAlert.innerHTML = msg : existingAlert.textContent = msg, 
        ...
        editableDiv && (targetID1 = className), ctrlDown || !outsideKeyPress && "readonly" != readonly) {
          ...
          cmdProcessTimeout = setTimeout(() => {
            ...
            chrome.storage.local.remove(currentID, function() {}), tmpAlert("Removed bookmark for this ticket!"); else if (value.startsWith("Bookmark")) {
              ...
              $('textarea[name="incident.short_description"]').css("background-color") !== data && ($('textarea[name="incident.short_description"]').css("background-color", data), 
              $('input[name="incident.short_description"]').css("background-color", data), 
              $('textarea[name="u_incident_task.short_description"]').css("background-color", data), 
              $('textarea[name="sc_task.short_description"]').css("background-color", data), 
              chrome.storage.local.set({
            ...
            chrome.storage.local.set({
              ...
              setTimeout(() => {
                ...
                tmpAlert("Copied: " + v));
              }, 1e3)) : (copyToClipboard(chatContext = value.slice(5).replace("ticketID", currentID).replace("jobID", jobID).replace("link", ticketLink).replace("shortDescription", $('textarea[name="incident.short_description"]').val())), 
              tmpAlert("Copied: " + chatContext));
            ...
            } else if (value.startsWith("Ask sandbox to suggest")) if (-1 != currentURL.indexOf("incident.do.html")) tmpAlert("This feature only works on the real incident page.", 5e3); else if (useBackgroundAPI || "" != chatSessionURL) {
              let desc = "User:\nShort description:\n" + $('textarea[name="incident.short_description"]').val() + "\nLong description:\n" + $('textarea[name="incident.description"]').val() + "\n", myPrompt1 = "";
              ...
              useBackgroundAPI ? (showLoadingSpinner(), tmpAlert("Uploading ticket in background...", 3e3), 
              chrome.runtime.sendMessage({
                greeting: "askChatgptBackground",
                question: myPrompt1
              }, function(response) {})) : chrome.storage.local.set({
            ...
            }) : tmpAlert("No message to edit.", 5e3); else if (value.startsWith("Summarize tickets0")) if ("" === myPrompt) tmpAlert("No conversaction is found"); else {
              let myPrompt1;
  ...
  $(document).ready(function() {
    ...
    loadCon();
    {
      var callback = function() {
        if (-1 == currentURL.indexOf("sandbox") && -1 == currentURL.indexOf("chatgpt.com") && -1 == currentURL.indexOf("cloude.ai") && -1 == currentURL.indexOf("openai.com") || chrome.storage.local.get([ "askChatgpt" ], function(result) {
          if (null != result.askChatgpt) {
            let lastOne = null;
            {
              ...
              const interval = setInterval(() => {
                ...
                if (texbox) {
                  var updateTimeout;
                  clearInterval(interval), -1 != currentURL.indexOf("sandbox") ? texbox.value = result.askChatgpt : -1 != currentURL.indexOf("chatgpt.com") && (texbox.innerText = result.askChatgpt), 
                  setTimeout(() => {
