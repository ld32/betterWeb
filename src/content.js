if (!window.__betterWebContentLoaded) {
  window.__betterWebContentLoaded = !0;
  try {
    chrome.runtime.getManifest().name.endsWith(" dev") && setInterval(function() {
      try {
        chrome.runtime.id || location.reload();
      } catch (e) {
        location.reload();
      }
    }, 1e3);
  ...
  urlMap.set("closedTicketColor", "white"), new Map()), shortcutMap = new Map(), shortcutCommentMap = new Map(), sentenceMap = new Map(), hideMap = new Map(), controlMap = new Map(), urlArray = {}, keywordArray = {}, controlArray = {}, shortcutArray = {}, sentenceArray = {}, hideArray = {}, fillArray = {}, favorArray = {}, fillButtonMap = new Map(), xpathMap = new Map(), controlButtonMap = new Map(), excelRowValue = "", lastText = "", lastTarget = null, chatTarget = null, sending = !1, timeOuts = {}, currentText = "", currentTxt = "", currentInstLoc = 0, firstName = "", myFirstName = "myFirstName", myID = "", myIDColumn = 0, classColumn = 0, classificationPrompt = "", classificationColor = [], greeting = "", listPagePaths = [], detailPagePaths = [], leftLinePositionListPage = 0, rightLinePositionListPage = 0, leftLinePositionDetailPage = 0, rightLinePositionDetailPage = 0, leftLineActionDetailPage = "", rightLineActionDetailPage = "", firstLineX = 0, firstLineX1 = 0;
  let userID = "";
  var firstNameWarning = "", hintIndex = 0, hintText = new Map(), lastInsertStart = 0, lastInsertLength = 0, lastTextWithoutInsert = "", isHiding = !1, insertLength = 0, insertStart = 0, ctrlDown = !1, cmdString = "", ctrlKey = 17, totalInsertLengthToday = 0, totalInsertLengthThisMonth = 0, todayTickets = {};
  let currentConfig = "", configMap = new Map(), editableDiv = !1, outsideKeyPress = !1, lastKeyTime = 0, alarm = new Map(), interv, cUrl = "", helpMsg = new Map(), savedDate = "", gotNewData = "no", isSelfClick = !1, currentID = "empty", ticketLink = "", autoRun = "", lastChar = "", lastKey = "", updateTimeout, myPrompt = "", jobID = "", chatContext = "", leftURL = "", rightURL = "", chatSessionURL = "", saveDrafTimeout = null, ticketURL = "";
  ...
  function qsa(selector, root = document) {
    ...
    var first = /^(.*):first$/.exec(selector);
    try {
      var els = (root || document).querySelectorAll(cssSelector(first ? first[1] : selector));
  ...
  function val(selector) {
    ...
    return selector ? selector.value : void 0;
  }
  ...
  function onDomReady(fn) {
    "loading" === document.readyState ? document.addEventListener("DOMContentLoaded", fn, {
      once: !0
    }) : setTimeout(fn, 0);
  }
  let lastWord = "";
  ...
  var delayTimeout, backgroundChangeable = !1, shortDescrtionYValue = 0, pageOpenTime = 0, pageCloseTime = 0, caretPosition = 0, searchTerms = [], allTicketIDs = [], orderedTicketNavigationStorageKey = "orderedTicketNavigation", domain = "", editingMode = !1, mouseX = 0, mouseY = 0, profileIsFound = !0;
  function updateEditControlMode(isEditing) {
    editingMode = !!isEditing, renderStatusPanel();
  }
  function setupEditControlModeStatus() {
    updateEditControlMode(!1);
  }
  ...
  function createHintBubble(baseText, suffixText, badgeText, options = {}) {
    var wrap = document.createElement("div"), panel = (wrap.setAttribute("data-bw-ui", ""), 
    document.createElement("div")), baseText = (panel.style.cssText = "font-family: Calibri, Arial, Helvetica, sans-serif; font-size: 12pt; color: white !important; display: inline-block; white-space: pre-wrap; background-color: black !important; padding: 10px;", 
    options.withBorder && (panel.style.border = "1px solid #606060"), panel.appendChild(document.createTextNode(baseText)), 
    document.createElement("span")), options = (baseText.style.setProperty("color", "#FF4C4C", "important"), 
    options.highlightId && (baseText.id = options.highlightId), baseText.appendChild(document.createTextNode(suffixText)), 
    document.createElement("span"));
    return options.id = "textPredictionTabHint", options.style.cssText = "width: 49px; height: 19px; border-radius: 2px; border-style: solid; border-width: 1px; border-color: #606060; white-space: nowrap; font-size: 0.7em; color: #32CD32 !important; padding-left: 3px; padding-right: 3px; padding-top: 1px; margin-left: 4px; position: relative; top: -2px;", 
    ...
    if (!(addedLength <= 0)) {
      ...
      chrome.storage.local.get({
        ...
        chrome.storage.local.set({
          ...
          chrome.runtime.lastError || tmpAlert(`Inserted ${addedLength} chars. Today: ${insertLengthToday}, Week: ${insertLengthThisWeek}, Month: ` + totalInsertLengthThisMonth, 2e4);
        });
      });
    }
  }
  function loadCon() {
    currentID = document.title.split(" ")[0], ticketLink = currentURL.split("&")[0], 
    chrome.storage.local.get({
      myConfigs: "myConfigs"
    }, function(ob) {
      if ("myConfigs" !== ob.myConfigs) {
        ...
        findURL ? loadConfig() : profileIsFound = !1;
      }
    });
  }
  function loadConfig() {
  ...
  function processData(ob) {
    ...
    searchTerms = splitWords(urlMap.get("searchTerms")), chatSessionURL = urlMap.get("chatSessionURL") ? urlMap.get("chatSessionURL") : "", 
    ...
    listPagePaths = splitWords(urlMap.get("listPagePaths")), (detailPagePaths = splitWords(urlMap.get("detailPagePaths"))).some(path => currentURL.includes(path))) {
      ...
      for (let i = 0; i < keys.length; i++) if (-1 == obj[keys[i]][0].indexOf("#")) {
        ...
        k.length < 4 && (/[^a-zA-Z0-9]/.test(k) || (helpMsg.set(k, "<p><strong>" + k + ": </strong>" + obj[keys[i]][0].split(" ").slice(1).join(" ") + "</p>"), 
      ...
      if (currentURL.startsWith("https://www.ncbi.nlm.nih.gov/tools/primer-blast/primertool.cgi")) return 0 < qsa("#userGuidedForm table tbody tr").length ? void chrome.storage.local.get([ "primerResults" ], function(result) {
        let re = result.primerResults, candi = currentURL;
        setTimeout(() => {
          chrome.storage.local.set({
            primerResults: re + candi + "\n"
          }, function() {}), clickEls('input[type="submit"]');
        }, 500);
        ...
        qsa(".prPairInfo table tbody").forEach(tbody => {
          ...
          rtext += index + "\tforward", row && row.querySelectorAll("td").forEach(td => {
            rtext += "\t" + txt(td);
          }), rtext += "\treverse", (row = tbody.querySelector("tr:nth-child(3)")) && row.querySelectorAll("td").forEach(td => {
            rtext += "\t" + txt(td);
          }), rtext += "\n";
      ...
      if (-1 != currentURL.indexOf("office.com/calendar")) (leftLineParts = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
          if (mutation.target, mutation.target && "ms-List-surface" === mutation.target.className) {
            ...
            clearInterval(interv), qsa(".ms-List-cell").forEach(function(cell) {
              cell = cell.innerHTML;
              if (-1 === cell.indexOf("All day")) {
                cell = cell.match(/(\d{1,2}:\d{2} [APM]{2}) to (\d{1,2}:\d{2} [APM]{2}), ([A-Za-z]+), ([A-Za-z]+ \d{1,2}, \d{4})/);
                if (cell) {
                  var div, startTime = cell[1], cell = (cell[2], cell[3], cell[4]);
                  ...
                  alarm.has(new Date(time).getTime()) ? interv = setInterval(() => {
                    ...
                    Math.floor(diff / 1e3 / 60)), diff = (diff -= 1e3 * mm * 60, 
                    Math.floor(diff / 1e3)), elements = document.getElementById(time); null != elements && 0 < elements.length; ) elements[0].parentNode.removeChild(elements[0]);
        ...
        if (-1 != currentURL.indexOf("outlook.office.com")) window.onfocus = function() {
          chrome.storage.local.get([ "searchEmail" ], function(result) {
            ...
            document.querySelector("#topSearchInput").dispatchEvent(new KeyboardEvent("keydown", {
              ...
              bubbles: !0,
              cancelable: !0
            }), document.querySelector("#topSearchInput").dispatchEvent(result), 
            document.querySelector('[data-testid="send-button"]'), qs(".form-inline.navpage-global-search.ng-non-bindable")?.requestSubmit(), 
            chrome.storage.local.remove("searchEmail", function() {}));
          });
        }; else if (-1 != currentURL.indexOf("chatgpt.come") || -1 != currentURL.indexOf("chatgpt.ai1")) {
          document.addEventListener("click", e => {
            e.target.nodeName;
          });
        ...
        } else if (detailPagePaths.some(path => currentURL.includes(path))) {
          leftLineParts = Array.from(document.querySelectorAll("label")).find(el => "Assigned to" === el.textContent.trim());
          let element = null;
          leftLineParts && (rightLineParts = leftLineParts.getAttribute("for"), 
          element = document.getElementById(rightLineParts)) && "input" === element.tagName.toLowerCase() && element.value, 
          ...
          document.addEventListener("click", e => {
            ...
            if ("rgba(0, 0, 0, 0)" === bgColor && ("DIV" === e.target.nodeName || "TD" === e.target.nodeName || -1 != currentURL.indexOf("testPage"))) {
              ...
              null != (targetElement = null != (targetElement = null != (targetElement = null != (targetElement = null != (targetElement = null != targetElement && 0 !== targetElement.length ? targetElement : qsa('input[name="incident.short_description"]')) && 0 !== targetElement.length ? targetElement : qsa('textarea[name="u_incident_task.short_description"]')) && 0 !== targetElement.length ? targetElement : qsa('textarea[name="sc_task.short_description"]')) && 0 !== targetElement.length ? targetElement : qsa('textarea[name="pm_project_task.description"]')) && 0 !== targetElement.length ? targetElement : qsa("#form_main")) && void 0 !== targetElement && (bgColor = targetElement[0].getBoundingClientRect(), 
              ...
              }), -1 != currentURL.indexOf("incident.do.html")) && chrome.runtime.sendMessage({
                greeting: "closeTab"
              }, response => {
                response && response.farewell && response.farewell;
              });
            ...
            hintText.clear())) : lastTarget = e.target, isSelfClick || ("TEXTAREA" === e.target.nodeName && "incident.description" != e.target.id ? (currentTxt = e.target.value, 
            currentInstLocation = e.target.selectionStart, updateEditControlMode(!0), 
            selectedText = e.target.value.substring(e.target.selectionStart, e.target.selectionEnd)) : -1 != currentURL.indexOf("incident_do") && "" == val('[name*="assigned_to"], [id*="assigned_to"]') || -1 != currentURL.indexOf("incident_task") && "" === val('input[name="sys_display.u_incident_task.assigned_to"]') || -1 != currentURL.indexOf("sc_task") && "" === val('input[name="sys_display.sc_task.assigned_to"]') ? (changeBackground(urlMap.get("newTicketColor")), 
            showEls(".sn-controls.row")) : "3" === val('select[name="incident.state"]') || "4" == val('select[name="u_incident_task.state"]') || "-5" == val('select[name="sc_task.state"]') ? changeBackground(urlMap.get("holdTicketColor")) : changeBackground("#FFFFCC"));
        ...
        setupKeyDownEventListerner(!1), setupKeyUpEventListerner(!1), document.addEventListener("mousemove", e => {
  ...
  function waitForLinks() {
    try {
      null != document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.getElementsByTagName("table") && setTimeout(() => {
        var ls = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelectorAll("a.linked.formlink");
  ...
  function getFirstName() {
    ...
    var cs = qsa(".h-card.h-card_md.h-card_comments");
    ...
    chrome.storage.local.get([ "downloadTickets", "rawTickets", "ticketDownloadCount" ], function(result) {
      if (result.downloadTickets) {
        ...
        ticketDownloadCount++, chrome.storage.local.set({
          ...
          }, 500) : (tmpAlert("Downloaded 10 tickets. Download complete.", 5e3), 
          chrome.storage.local.remove("downloadTickets", function() {})));
        }));
      }
    }), "" == mostRecentMessage && (mostRecentMessage = val('textarea[name="incident.description"]')), 
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      "phi3-stream-chunk" === message.type ? (tmpAlert("Received chunk: " + message.chunk, 3e3), 
      hideLoadingSpinner()) : -1 != currentURL.indexOf("service-now") && message.greeting.startsWith("answerFromChatgptBackground") && (hideLoadingSpinner(), 
      void 0 !== message.chatAnswer) && "" !== message.chatAnswer.trim() && ((message = createHintBubble(message.chatAnswer, "", "tab")).className = "addedhere", 
      document.getElementById("activity-stream-comments-textarea").parentElement.append(message));
    });
    let el;
    if ((void 0 === firstName || firstName.length < 2) && (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector("#bf1d96e3c0a801640190725e63f8ac80 > div:nth-child(2) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2) > input:nth-child(3)").value : el = qsa("#bf1d96e3c0a801640190725e63f8ac80 > div:nth-child(2) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2) > input:nth-child(3)"), 
    null != (el = null === el ? qsa("#4fc4979ec0a8016401e142a5a0c599ce > div: nth - child(1) > div: nth - child(1) > div: nth - child(2)") : el) && null != el[0] && null != el[0].value && 5 < el[0].value.length && (firstName = el[0].value.split(/\s+/)[0], 
    sureAboutFirstName = !0), void 0 === firstName || firstName.length < 2) && (t = val("#sc_task\\2e request\\2e requested_for_label")) && (firstName = t.split(/\s+/)[0], 
    sureAboutFirstName = !0), void 0 === firstName || firstName.length < 2) null != (el = qsa("#bf1d96e3c0a801640190725e63f8ac80 > div:nth-child(2) > div:nth-child(1) > div:nth-child(6) > div:nth-child(2) > input:nth-child(2)")) && null != el[0] && 5 < el[0].value.length && (firstName = el[0].value.split(/\s+/)[0], 
    ...
    if (void 0 === firstName || firstName.length < 2) {
      ...
      0 != el.length && 2 < (firstName = el[0].innerHTML.split("value=")[1].split('"')[1].split(" ")[0]).length && (sureAboutFirstName = !0);
    }
    if (void 0 === firstName || firstName.length < 2) {
      let tMail = null;
      if (newFormat ? tMail = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector("#bf1d96e3c0a801640190725e63f8ac80 > div:nth-child(2) > div:nth-child(1) > div:nth-child(7) > div:nth-child(2) > input").value : el = document.getElementById("incident.u_caller_email"), 
      ...
      if (void 0 === firstName || firstName.length < 2) {
        var t = val("#sc_task\\2e u_preferred_email");
        if (t) {
          let firstName1 = "";
    ...
    if (void 0 === firstName || 2 < firstName.length && !sureAboutFirstName) {
      let des = null;
      null != (des = (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector('textarea[name="incident.description"]') : qsa('textarea[name="incident.description"]')[0]).value) && 0 < des.length && -1 == des.indexOf(firstName) && (firstNameWarning = "First name from desc may not correct: " + firstName);
    }
  ...
  function shortDescriptionReadOnly() {
    !document.activeElement || "INPUT" !== document.activeElement.tagName && "TEXTAREA" !== document.activeElement.tagName && "SELECT" !== document.activeElement.tagName || document.activeElement.blur();
  }
  function shortDescriptionEditable() {
    try {
      "" != val('input[name="incident.short_description"]') && removeAttr('input[name="incident.short_description"]', "readonly"), 
      "" != val('textarea[name="incident.short_description"]') && removeAttr('textarea[name="incident.short_description"]', "readonly"), 
      "" != val('textarea[name="incident.description"]') && removeAttr('textarea[name="incident.description"]', "readonly"), 
      "" != val('textarea[name="u_incident_task.short_description"]') && removeAttr('textarea[name="u_incident_task.short_description"]', "readonly"), 
      "" != val('textarea[name="sc_task.short_description"]') && removeAttr('textarea[name="sc_task.short_description"]', "readonly");
    } catch (e) {}
  }
  function makeDescriptionReadOnly() {
    var element = qsa('textarea[name="incident.description"]')[0];
    if (element = element || qsa('textarea[name="sc_task.description"]')[0]) {
      var arr = element.value.split(/\n\s+|\s+|<|>|\)|\(/);
      for (let i = 0; i < arr.length; i++) (arr[i].endsWith(",") || arr[i].endsWith(".")) && (arr[i] = arr[i].slice(0, -1)), 
      !arr[i].startsWith("https://") && !arr[i].startsWith("http://") || arr[i].endsWith("complianceline") ? /^\d{8}$/.test(arr[i]) && (jobID = jobID + " " + arr[i]) : appendSafeExternalLink(qs('textarea[name="incident.description"]')?.parentElement, arr[i]);
    }
    ...
    try {
      "" != val('input[name="incident.short_description"]') && setAttr('input[name="incident.short_description"]', "readonly", "readonly"), 
      "" != val('textarea[name="incident.short_description"]') && setAttr('textarea[name="incident.short_description"]', "readonly", "readonly"), 
      "" != val('textarea[name="incident.description"]') && setAttr('textarea[name="incident.description"]', "readonly", "readonly"), 
      "" != val('textarea[name="u_incident_task.short_description"]') && setAttr('textarea[name="u_incident_task.short_description"]', "readonly", "readonly"), 
      "" != val('textarea[name="sc_task.short_description"]') && setAttr('textarea[name="sc_task.short_description"]', "readonly", "readonly");
    } catch (e) {}
    for (key in chrome.storage.local.get([ currentID + "color" ], function(result) {
      var assignedTo;
      void 0 !== result[currentID + "color"] && (result = result[currentID + "color"], 
      assignedTo = val('[name*="assigned_to"], [id*="assigned_to"]'), "green" === result && assignedTo && -1 != assignedTo.indexOf(myFirstName) || showBookmarkTag(result));
    ...
    chrome.storage.local.set({
      ...
      if (result.classifyTicketList && 0 < result.classifyTicketList.length) if ("" == chatSessionURL) alert("Please set up the value for chatSessionURL in option page, or enable 'Use Background API' in options."); else {
        ...
        for (let i = 0; i < newArry.length; i++) if (newArry[i].split(":")[0] != currentID) if (void 0 === await getFromStorage(newArry[i].split(":")[0] + "description")) {
          ...
          -1 != currentURL.indexOf("testPage") && (url = domain + "/testPage/incident.do.html?sys_id=" + sisid), 
          chrome.runtime.sendMessage({
            greeting: "Open " + url
          }, function(response) {});
          break;
        ...
        if (allDone) {
          ...
          sendToChatSession(myPromptAll = myPromptAll + "\n" + tics + "\n" + classificationPrompt + "\nThanks."), 
          chrome.runtime.sendMessage({
  ...
  function showClassificationDecisionModal(classificationResult, description, classificationKey) {
    return new Promise(function(resolve) {
      ...
      document.createElement("div")), closeBtn = (footer.style.display = "flex", 
      footer.style.gap = "8px", footer.style.justifyContent = "flex-end", footer.style.marginTop = "12px", 
      document.createElement("button")), removeBtn = (closeBtn.textContent = "Close", 
      ...
      closeBtn.style.background = "#f6f6f6", closeBtn.style.cursor = "pointer", 
  ...
  function parseStoredTicketUrl(rawUrl, baseUrl) {
    ...
    try {
      var url = new URL(rawUrl, baseUrl || window.location.href);
      ...
      if (!sysId) {
        ...
        var sysIdMatch = decodedUrl.match(/[?&]sys_id=([^&#]+)/i);
        sysIdMatch && (sysId = sysIdMatch[1]);
      }
      return sysId && /^https?:$/.test(url.protocol) ? {
        url: url.href,
        sysId: sysId
      } : null;
    } catch (_error) {
  ...
  function getTicketIdFromListLink(link) {
    var linkText = link && link.textContent ? link.textContent.trim() : "", link = link && link.getAttribute && link.getAttribute("aria-label") || "", ticketMatch = (linkText + " " + link).match(/\b(?:INC|ITSK|TASK|RITM|REQ)\d+\b/i);
    return ticketMatch ? ticketMatch[0].toUpperCase() : 0 < (ticketMatch = link.trim().split(/\s+/)).length ? ticketMatch[ticketMatch.length - 1] : linkText;
  }
  function saveOrderedTicketNavigation(links, sourceUrl) {
    const tickets = [], seenSysIds = new Set();
    if (Array.from(links || []).forEach(link => {
      var parsed = parseStoredTicketUrl(link.href || link.getAttribute && link.getAttribute("href"), sourceUrl);
      parsed && !seenSysIds.has(parsed.sysId) && (seenSysIds.add(parsed.sysId), 
      tickets.push({
        ticketId: getTicketIdFromListLink(link),
        sysId: parsed.sysId,
        url: parsed.url
      }));
    }), 0 === tickets.length) return Promise.resolve(!1);
    const storageUpdate = {
    ...
    return tickets.forEach(ticket => {
      ticket.ticketId && (storageUpdate[ticket.ticketId + "sysid"] = ticket.sysId);
    }), new Promise(resolve => {
      chrome.storage.local.set(storageUpdate, function() {
        chrome.runtime.lastError ? resolve(!1) : resolve(!0);
      });
    });
  }
  function getServiceNowEmbeddedFrames() {
    var frames = [], classicFrame = document.getElementById("gsft_main");
    classicFrame && frames.push(classicFrame);
    try {
      var macroponent = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010"), workspaceFrame = macroponent && macroponent.shadowRoot ? macroponent.shadowRoot.querySelector("iframe") : null;
      workspaceFrame && !frames.includes(workspaceFrame) && frames.push(workspaceFrame);
    } catch (_error) {}
    return frames;
  }
  function getCurrentTicketLocation() {
    for (const frame of getServiceNowEmbeddedFrames()) try {
      var parsed = parseStoredTicketUrl(frame.contentWindow && frame.contentWindow.location ? frame.contentWindow.location.href : frame.src, window.location.href);
      if (parsed) return {
        ...parsed,
        frame: frame
      };
    ...
    var parsedWindowUrl = parseStoredTicketUrl(window.location.href, window.location.href);
    return parsedWindowUrl ? {
      ...parsedWindowUrl,
      frame: null
    } : null;
  }
  function isSafeStoredTicketUrl(rawUrl) {
    try {
      var targetUrl = new URL(rawUrl, window.location.href), parsed = parseStoredTicketUrl(targetUrl.href, window.location.href), isTicketPath = /\/incident\.do(?:\.html)?$/i.test(targetUrl.pathname);
  ...
  async function goToStoredNextTicket() {
    var navigation = await getFromStorage(orderedTicketNavigationStorageKey);
    if (!navigation || !Array.isArray(navigation.tickets) || 0 === navigation.tickets.length) return !1;
    const currentLocation = getCurrentTicketLocation();
    let currentIndex = -1;
    if (-1 === (currentIndex = -1 === (currentIndex = currentLocation && currentLocation.sysId ? navigation.tickets.findIndex(ticket => String(ticket.sysId) === String(currentLocation.sysId)) : currentIndex) && currentID && "empty" !== currentID ? navigation.tickets.findIndex(ticket => String(ticket.ticketId || "").toUpperCase() === String(currentID).toUpperCase()) : currentIndex)) return !1;
    if (currentIndex >= navigation.tickets.length - 1) return tmpAlert("Last ticket in saved list.", 3e3), 
    !0;
    navigation = navigation.tickets[currentIndex + 1];
    if (!navigation || !isSafeStoredTicketUrl(navigation.url)) return !1;
    try {
      return currentLocation && currentLocation.frame && currentLocation.frame.contentWindow ? (currentLocation.frame.contentWindow.location.assign(navigation.url), 
      setTimeout(() => {
        newFormat && processIFrame(null);
      }, 4e3)) : window.location.assign(navigation.url), !0;
    } catch (error) {
      return !1;
    }
  }
  async function processIFrame(iframe) {
    ...
    if (null != iframe || rootDoc === document) {
      ...
      if (myIDColumn = th ? th.cellIndex - 1 : 3, ls = rootDoc && rootDoc.body ? rootDoc.body.querySelectorAll("a.linked.formlink") : null, 
      lsTime = rootDoc && rootDoc.body ? rootDoc.body.querySelectorAll(".datex.date-timeago") : null, 
      null != ls && 0 < ls.length) {
        let listSourceUrl = window.location.href;
        try {
          rootDoc.location && rootDoc.location.href && (listSourceUrl = rootDoc.location.href);
        } catch (_error) {}
        await saveOrderedTicketNavigation(ls, listSourceUrl);
        th = new Date(), th = th.getFullYear() + `-${String(th.getMonth() + 1).padStart(2, "0")}-` + String(th.getDate()).padStart(2, "0");
        void 0 !== todayTickets && todayTickets.today == th || (todayTickets = {});
      }
      for (let i = 0; i < ls.length; i++) {
        var lastSpaceIndex = ls[i].getAttribute("aria-label").lastIndexOf(" "), lastSpaceIndex = ls[i].getAttribute("aria-label").substring(lastSpaceIndex + 1), [ datePart, timePart ] = lsTime[i].getAttribute("title").split(" ");
        const [ month, day, year ] = datePart.split("-");
        var [ datePart, timePart, seconds ] = timePart.split(":"), datePart = new Date(year, month - 1, day, datePart, timePart, seconds).getTime();
        allTicketIDs.push(lastSpaceIndex);
        let key = lastSpaceIndex + "sysid", sysid = ls[i].getAttribute("href").split("sys_id=")[1].split("&")[0];
        chrome.storage.local.set({
          [key]: sysid
        }, function() {}), void 0 !== todayTickets && lastSpaceIndex in todayTickets && datePart < todayTickets[lastSpaceIndex] && (ls[i].style.color = "red", 
        ls[i].style.backgroundColor = "yellow"), ls[i].target = "_blank";
        timePart = ls[i].closest("tr"), seconds = timePart.querySelectorAll("td"), 
        datePart = (seconds[2].querySelector("a.linked.formlink").getAttribute("href"), 
        seconds[myIDColumn + 1]);
        if (-1 === txt(datePart).toLowerCase().indexOf(myID.toLowerCase())) {
          datePart.style.color = "red";
          const link = datePart.querySelector("a");
          link && (link.style.color = "red");
        }
        var [ seconds, , ,  ] = await Promise.all([ getFromStorage(lastSpaceIndex + "color"), getFromStorage(lastSpaceIndex + "description"), getFromStorage(lastSpaceIndex + "classificationResult") ]);
        void 0 === seconds || "green" === seconds && -1 != txt(timePart).indexOf(myFirstName) || (ls[i].style.backgroundColor = seconds);
      }
        ...
        ev.target.style.color = "red"), "rgba(0, 0, 0, 0)" === backgroundColor || "rgb(255, 255, 255)" === backgroundColor || "rgb(245, 245, 245)" === backgroundColor || "rgb(238, 238, 238)" === backgroundColor) {
          ...
          if (backgroundColor > windowWidth * leftLinePositionListPage && backgroundColor < windowWidth * rightLinePositionListPage) isValidURL(leftURL) ? window.location.href = leftURL : alert("Please set value for variable leftURL in option page."); else if (backgroundColor > windowWidth * rightLinePositionListPage) isValidURL(rightURL) ? window.location.href = rightURL : alert("Please set value for variable rightURL in option page."); else if ("A" != ev.target.nodeName) {
            ...
            if (backgroundColor) {
              ...
              link = getLinkFromRow(backgroundColor)), link && chrome.runtime.sendMessage({
      ...
      rootDoc = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010");
      if (null == rootDoc || null == rootDoc.shadowRoot || null == rootDoc.shadowRoot.querySelector("iframe") || null == rootDoc.shadowRoot.querySelector("iframe").contentWindow.document.body || null == rootDoc.shadowRoot.querySelector("iframe").contentWindow.document.body.querySelectorAll("a.linked.formlink") || 0 == rootDoc.shadowRoot.querySelector("iframe").contentWindow.document.body.querySelectorAll("a.linked.formlink").length) setTimeout(() => {
        ...
        null != ls && 0 < ls.length) {
          ...
          try {
            workspaceListUrl = rootDoc.shadowRoot.querySelector("iframe").contentWindow.location.href;
          } catch (_error) {}
          ...
          chrome.storage.local.get([ "bookmarkTickets" ], function(result) {
            if (!chrome.runtime.lastError) {
              ...
              for (var i = 0; i < ls.length; i++) {
                ...
                -1 === txt(day).toLowerCase().indexOf(myID.toLowerCase()) && (day.style.color = "red", 
  ...
  function processButton0(newTabOpened, labelIndex, labelValuesPairs) {
    if (newTabOpened) chrome.storage.local.get([ "newTabOpened" ], function(result) {
      ...
      } else if (labelValuesPairs[labelIndex].startsWith("Click")) clickEls(labelValuesPairs[labelIndex].split(" ")[1]); else if (labelValuesPairs[labelIndex].startsWith("Delay")) {
        ...
        if (xpaths[0].startsWith("button")) newTabOpened = !0, chrome.storage.local.set({
          ...
          for (let t1 = 1; t1 < labelValuesPair.length; t1++) {
            ...
            if (newFormat) {
              ...
              (null != o1 && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(3)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(4)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(5)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(6)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(7)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(8)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(9)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(10)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(11)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(12)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(13)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(14)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(15)")) && o1.text === va || null != (o1 = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0] + " option:nth-child(16)")) && o1.text === va) && (o1.selected = "SELECTED");
            } else [ ...qsa(xpaths[0] + " option") ].filter(opt => (txt(opt), txt(opt) === va)).forEach(opt => {
              opt.selected = !0;
            });
          }
          (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0]) : qsa(xpaths[0])[0]).dispatchEvent(new Event("change"));
        } else if (xpaths[0].startsWith("textarea")) {
          va = labelValuesPair[1];
          newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0]).value = va : qsa(xpaths[0])[0].value = va;
        } else if (xpaths[0].startsWith("input")) {
          let originalValue = null;
          null != (originalValue = newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[0]).value : val(xpaths[0])) && 0 < originalValue.length && "Assignment group" != labelValuesPair[0].trim() || "On hold expiration date" === labelValuesPair[0] ? (((va = labelValuesPair[1]).endsWith("week") || va.endsWith("weeks")) && (timestamp = new Date(), 
          va = function(date) {
          ...
          timestamp))), setVal(xpaths[0], va), qsa(xpaths[0]).forEach(el => el.dispatchEvent(new Event("change", {
            bubbles: !0
          }))), qs(xpaths[0])?.focus()) : 3 == xpaths.length && xpaths[1].startsWith("button") && (newTabOpened = !0, 
          chrome.storage.local.set({
            assignTicket: xpaths[2] + ":" + labelValuesPair[1].trim()
          }, function() {}), chrome.storage.local.set({
            newTabOpened: "yes"
          }, function() {
            (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(xpaths[1]) : qsa(xpaths[1])[0]).click();
          }));
        }
      }
    }
  ...
  function processButton(newTabOpened, labelIndex, labelValuesPairs) {
    if (-1 < labelIndex && tmpAlert(labelIndex + " : " + labelValuesPairs[labelIndex], 1e3), 
    newTabOpened) chrome.storage.local.get([ "newTabOpened" ], function(result) {
      null != result.newTabOpened && "no" == result.newTabOpened && (newTabOpened = !1);
    }); else {
      if (++labelIndex === labelValuesPairs.length) return;
      if (1 == labelValuesPairs[labelIndex].length) document.dispatchEvent(new KeyboardEvent("keyup", {
  ...
  function checkMonth(index, rs, fileName, r) {
    var currentDate = qsa(".monthAndYear-180")[0].firstChild.textContent, month = currentDate.split(/\s+/)[0];
    ...
    -1 != r.date.indexOf(month) ? setTimeout(() => {
      ...
      setTimeout(() => {
        qsa(".flexContainer-158")[4].click(), setTimeout(() => {
          qsa(".ms-TextField-field")[1].value = r.topic, qsa(".ms-Button.kIT8h.ms-Button--icon.ms-ComboBox-CaretDown-button")[0].click(), 
          setTimeout(() => {
            var ops = qsa(".ms-Button.ms-Button--action.ms-Button--command.ms-ComboBox-option");
            ...
            setTimeout(() => {
              ...
              qsa("#innerRibbonContainer div:nth-child(4) button")[1].click(), setTimeout(() => {
                qsa(".ms-FocusZone li button")[0].click(), setTimeout(() => {
                  var meetingID = qsa("tbody")[1].textContent.split("?pwd")[0].split("j/")[1];
    ...
    }, 500) : (qsa(".headerIconButton-182")[1].click(), setTimeout(() => {
  ...
  function processCalendarRow(index, rs, fileName) {
    if (++index != rs.length) chrome.runtime.sendMessage({
      ...
      let r = rs[index];
      qsa(".ms-Button.ms-Button--default.Ca7_4.root-209")[0].click(), setTimeout(() => {
        checkMonth(index, rs, fileName, r);
      }, 1e3);
    }
  }
  ...
  function hintWindow(msg) {
    ...
    el.setAttribute("id", "tempAlertDiv1"), el.setAttribute("data-bw-ui", ""), el.style.cssText = `
    position: fixed;
    ...
    z-index: 1000;
`, el.textContent = msg;
    let x = mouseX + -60, y = mouseY - el.offsetHeight + 20;
    x + el.offsetWidth > window.innerWidth && (x = window.innerWidth - el.offsetWidth + 60), 
    y < 0 && (y = mouseY + -20), el.style.left = x + "px", el.style.top = y + "px", 
    document.body.appendChild(el);
  }
  function chatSessionHost() {
    try {
      return new URL(chatSessionURL).host;
    } catch (e) {
      return chatSessionURL;
    }
  }
  function confirmSendToChatSession(promptText) {
    promptText = null == promptText ? "" : String(promptText), promptText = 700 < promptText.length ? promptText.slice(0, 700) + "\n\n... and " + (promptText.length - 700) + " more characters" : promptText;
    return window.confirm("Send this to " + chatSessionHost() + "?\n\nThis text leaves your computer and is submitted to that site automatically.\n\n----------------------------------------\n" + promptText + "\n----------------------------------------");
  }
  function sendToChatSession(promptText) {
    return confirmSendToChatSession(promptText) ? (chrome.storage.local.set({
      askChatgpt: promptText
    }, function() {
      chrome.runtime.sendMessage({
        greeting: "Open " + chatSessionURL + " in new tab"
      }, function(response) {
        chrome.runtime.lastError;
      });
    }), !0) : (tmpAlert("Not sent to " + chatSessionHost() + ".", 2e3), !1);
  }
  onDomReady(function() {
    pageOpenTime = new Date().getTime(), currentURL = window.location.href, domain = currentURL.startsWith("file://") ? "file://Users/ld32/projects/betterWebPrivate" : -1 != currentURL.indexOf("testPage") ? "https://ld32.github.io/betterWeb" : "https://" + currentURL.match(/https:\/\/([^\/]+)/)[1], 
    loadCon();
    {
      var callback = function() {
        if (-1 == currentURL.indexOf("chatgpt.com") && -1 == currentURL.indexOf("cloude.ai") && -1 == currentURL.indexOf("openai.com") || chrome.storage.local.get([ "askChatgpt" ], function(result) {
          if (null != result.askChatgpt) {
            let lastOne = null;
            {
              ...
              const interval = setInterval(() => {
                ...
                if (texbox) {
                  var updateTimeout;
                  clearInterval(interval), -1 != currentURL.indexOf("chatgpt.com") && (texbox.innerText = result.askChatgpt), 
                  setTimeout(() => {
                    ...
                    elements || (updateTimeout = setTimeout(() => {
                      observer.disconnect(), null != lastOne && chrome.storage.local.get([ "classifyTicketList" ], function(result) {
                        if (result.classifyTicketList && 0 < result.classifyTicketList.length) {
                          ...
                          for (let i = 0; i < tics.length; i++) if (-1 != tics[i].indexOf(" Classification: ")) {
          ...
          chrome.storage.local.get([ "todayTickets" ], function(result) {
            ...
            }, 8e3) : (clickEls(".input-group-addon-transparent.icon-search.sysparm-search-icon"), 
            setVal("#sysparm_search", result.searchTicket), qs(".form-inline.navpage-global-search.ng-non-bindable")?.requestSubmit()), 
            chrome.storage.local.remove("searchTicket", function() {}), setTimeout(() => {
              ...
              -1 != (iframeURL = iframe.contentWindow.location.href).indexOf("incident.do") ? chrome.storage.local.get("followUpWorking", function(result) {
                null != result.followUpWorking ? chrome.runtime.sendMessage({
                  greeting: "Open " + iframeURL + " in new tab"
                }, function(response) {}) : chrome.runtime.sendMessage({
                  greeting: "Open " + iframeURL
                }, function(response) {});
              }) : processIFrame(iframe);
        ...
        }), detailPagePaths.some(path => currentURL.includes(path)) && (setupEditControlModeStatus(), 
        null != document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010") && alert("Please disable 'Always show top navigation' in 'Settings' > 'Display' or enable 'Turn off Next Experience' in 'Settings' > 'User experience' > 'Turn off Next Experience'"), 
        chrome.storage.local.get([ "delayKeyPressAfterReload" ], function(result) {
          if (!chrome.runtime.lastError) {
        ...
        }, 100), chrome.storage.local.get([ currentID + "_userID" ], function(result) {
          chrome.runtime.lastError || (userID = result[currentID + "_userID"]);
        }));
      };
      let waitCount = 0;
      const interval = setInterval(() => {
        "" != ticketURL && void 0 !== ticketURL ? (clearInterval(interval), callback()) : (20 <= ++waitCount || 0 == profileIsFound) && clearInterval(interval);
      }, 500);
    }
  });
  const ALERT_ALLOWED_TAGS = new Set([ "P", "BR", "DIV", "SPAN", "STRONG", "B", "EM", "I", "U", "UL", "OL", "LI", "CODE", "PRE" ]);
  function sanitizeAlertHtml(html) {
    var html = new DOMParser().parseFromString(String(html), "text/html"), out = document.createDocumentFragment();
    return function copy(from, to) {
      for (const node of Array.from(from.childNodes)) {
        var el;
        node.nodeType === Node.TEXT_NODE ? to.appendChild(document.createTextNode(node.nodeValue)) : node.nodeType === Node.ELEMENT_NODE && (ALERT_ALLOWED_TAGS.has(node.tagName) ? (el = document.createElement(node.tagName.toLowerCase()), 
        copy(node, el), to.appendChild(el)) : copy(node, to));
      }
    }(html.body, out), out;
  }
  function tmpAlert(msg, duration = 1e3, renderHtml = !1) {
    ...
    el.setAttribute("data-bw-ui", ""), el.style.cssText = "position:fixed;top:10%;left:10%;background-color:black !important;padding:10px;color:white !important;border-radius:5px;font-size:12pt;z-index: 1000;", 
    document.createElement("div"));
    existingAlert.style.cssText = "font-family: Calibri, Arial, Helvetica, sans-serif; font-size: 12pt; color: white !important; display: inline-block; white-space: pre-wrap; background-color: black !important; padding: 10px;", 
    renderHtml ? existingAlert.appendChild(sanitizeAlertHtml(msg)) : existingAlert.textContent = msg, 
    el.appendChild(existingAlert), setTimeout(function() {
      el.parentNode && el.parentNode.removeChild(el);
    }, duration), document.body.appendChild(el);
  }
  function setupIncidentRowHoverHint(rootBody) {
    rootBody && "true" !== rootBody.dataset.betterWebRowHoverHintBound && (rootBody.dataset.betterWebRowHoverHintBound = "true", 
    rootBody.addEventListener("mouseover", function(event) {
      var row = event.target.closest("tbody tr");
      row && row.querySelector("a.linked.formlink") && (event.relatedTarget && row.contains(event.relatedTarget) || tmpAlert("Click anywhere in this row to open the record. After clicking, incident ID will change color for the day until someone else replies.", 2e3));
    }));
  }
  function showLoadingSpinner(message = "Processing...") {
    ...
    spinner.setAttribute("data-bw-ui", ""), spinner.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background-color:rgba(0,0,0,0.85) !important;padding:30px;color:white !important;border-radius:10px;font-size:14pt;z-index:10000;text-align:center;min-width:250px;box-shadow:0 4px 20px rgba(0,0,0,0.5);", 
  ...
  function processInsert(e) {
    ...
    } else if (!editableDiv && 0 < insertLength) {
      e.target.value = currentText;
      var keyC = new Event("input", {
        bubbles: !0,
        cancelable: !0
      });
      if (e.target.dispatchEvent(keyC), "function" == typeof e.target.setSelectionRange) {
        let caretPos = insertStart + insertLength;
        (caretPos = caretPos < 0 ? 0 : caretPos) > currentText.length && (caretPos = currentText.length), 
  ...
  function setupKeyUpEventListerner(isIFrame) {
    var doc = document;
    (doc = isIFrame ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body : doc).addEventListener("keyup", function(e) {
      if (caretPosition = getCaretPosition(e.target), "ArrowLeft" != e.key && "ArrowRight" != e.key && "ArrowUp" != e.key && "ArrowDown" != e.key && "Shift" != e.key && "Alt" != e.key) if (e.keyCode == ctrlKey) ctrlDown = !1; else if ("Backspace" === e.key && caretPosition == lastInsertStart + lastInsertLength - 1) {
        if (insertLength = 0, insertStart = lastInsertStart, lastText = currentText = lastTextWithoutInsert, 
        e.target.value = currentText, "function" == typeof e.target.setSelectionRange) {
          let caretPos = insertStart;
          (caretPos = caretPos < 0 ? 0 : caretPos) > currentText.length && (caretPos = currentText.length), 
          e.target.setSelectionRange(caretPos, caretPos);
        }
      } else if ("Tab" === e.key && 1 === hintText.size) e.target.value = currentText + hintText.get(1) + " ", 
      ...
      hintText.clear(), tmpHintOn = !1, clearTimeout(cmdProcessTimeout); else {
        ...
        editableDiv && (targetID1 = className), ctrlDown || !outsideKeyPress && "readonly" != readonly) {
          ...
          updateInsertLength(insertLength = currentText.length), e.target.dispatchEvent(new Event("change")); else if (lastWord[0] != lastWord[0].toUpperCase() && lastWord.length < 10 && 0 < lastWord.length) {
            new Date().getTime() - lastKeyTime < 500 && clearTimeout(cmdProcessTimeout);
          ...
          processInsert(e), saveDrafTimeout = setTimeout(() => {
            chrome.storage.local.set({
              ...
              chrome.runtime.lastError || result[currentID + "color"] || (showBookmarkTag("green"), 
              chrome.storage.local.set({
          ...
          cmdProcessTimeout = setTimeout(() => {
            if (value = controlMap.get(cmdString), tmpAlert(cmdString + ": " + (value = null == value ? "" : value), 2e3), 
            cmdString = "", value.startsWith("Save selection to profile") && 10 < tex.length) {
              for (;;) {
                var tenure = prompt('Please enter the shortcut for "' + tex + '". Leave blank if you want to save as a sentence without shortcut.');
                if (!(null != tenure && 0 < tenure.length)) return;
                if (!shortcutMap.has(tenure.split(/\s+/)[0])) break;
              ...
              c = 0;
              for (let k of sentenceMap.keys()) sentenceArray[c++] = [ sentenceMap.get(k), k ];
              var data = {}, data = (data[1] = urlArray, data[2] = keywordArray, 
              ...
              var chatContext = document.getElementById("activity-stream-comments-textarea").value;
              if ("" === chatContext) {
              ...
              } else favorArray[Object.entries(favorArray).length] = [ currentID, chatContext ];
              tmpAlert("ticket is bookmakred"), showBookmarkTag(urlMap.get("bookmarkTicketColor"));
            ...
            chrome.runtime.sendMessage({
              greeting: "closeTab"
            }, function(response) {}); else if (value.startsWith("Remove bookmark")) showBookmarkTag("white"), 
            chrome.storage.local.remove(currentID, function() {}), tmpAlert("Removed bookmark for this ticket!"); else if (value.startsWith("Bookmark")) {
              data = value.split(/:/)[1].trim();
              currentBookmarkColor !== data && (showBookmarkTag(data), chrome.storage.local.set({
                [currentID + "color"]: data
              }, function() {}), tmpAlert("Bookmark color is set to " + data), setTimeout(() => {
                chrome.storage.local.get([ currentID + "color" ], function(result) {
              ...
              [currentID + "draft"]: currentText
            }, function() {}), 0 == currentText.length || currentText.trimStart().startsWith(greeting.split(" ")[0]) && currentText.trim().endsWith(myFirstName) || -1 == currentURL.indexOf("service") || "activity-stream-comments-textarea" !== lastTarget.id ? newFormat ? (document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(urlMap.get("submit")).click(), 
            setTimeout(() => {
              processIFrame(null);
              ...
              chrome.storage.local.set({
                ...
                chrome.runtime.lastError || qsa(urlMap.get("submit"))[0].click();
              }));
            }) : (alert("Not starting with greeting, or signature not found in your message!"), 
            showAll()); else if (value.startsWith("Click")) {
              value.startsWith("Click .icon-arrow") && chrome.storage.local.set({
                upDownClicked: "yes"
              }, function() {}), changeBackground(urlMap.get("inActionColor"));
              const clickExistingControl = function() {
                var macroponent, selector = value.split(/\s+/)[1];
                newFormat ? (macroponent = (macroponent = (macroponent = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010")) && macroponent.shadowRoot ? macroponent.shadowRoot.querySelector("iframe") : null) && macroponent.contentWindow && macroponent.contentWindow.document.body ? macroponent.contentWindow.document.body.querySelector(selector) : null) && (macroponent.click(), 
                setTimeout(() => {
                  processIFrame(null);
                }, 4e3)) : qsa(selector)[0].click();
              };
              value.startsWith("Click .icon-arrow-down") ? goToStoredNextTicket().then(handled => {
                handled || clickExistingControl();
              }).catch(error => {
                clickExistingControl();
              }) : clickExistingControl();
            } else if (value.startsWith("CheckAndDelayClick submitButton")) changeBackground(urlMap.get("inActionColor")), 
            sending || (sending = !0, (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(urlMap.get("submit")) : qsa(urlMap.get("submit"))[0]).click()); else if (value.startsWith("Open")) changeBackground(urlMap.get("inActionColor")), 
            chrome.runtime.sendMessage({
              greeting: value
            ...
            setZoomMeetings(); else if (value.startsWith("fill")) {
              ...
              chatContext = value.split(/\n/);
              processButton(!1, 0, chatContext);
            ...
            chrome.storage.local.set({
              ...
              tmpAlert("Tickets will be stop downloadeding.", 4e3);
            }); else if (value.startsWith("Copy:")) if (-1 != value.indexOf("userID")) "" === userID || void 0 === userID ? (button = document.querySelector('[id="viewr.incident.caller_id"]')).disabled || (button.click(), 
            setTimeout(() => {
              var v;
              ...
              tmpAlert("Copied: " + v), chrome.storage.local.set({
                [curentID + "_userID"]: userID
              }, function() {}));
            }, 1e3)) : (copyToClipboard(chatContext = value.slice(5).replace("userID", userID).replace("ticketID", currentID).replace("jobID", jobID)), 
            tmpAlert("Copied: " + chatContext)); else {
              ...
              copyToClipboard(v), tmpAlert("Copied: " + v);
            } else if (value.startsWith("Chat")) window.postMessage("testMessage", "*"); else if (value.startsWith("Ask AI server to classify conversation")) {
              let desc = "User:\nShort description:\n" + val('textarea[name="incident.short_description"]') + "\nLong description:\n" + val('textarea[name="incident.description"]') + "\n";
              ticketData = {
                ticket_id: currentID,
                conversation: desc
              }, tmpAlert("Asking AI to classify conversation in background...", 3e3), 
              chrome.runtime.sendMessage({
                type: "classify-conversation",
                conversation: ticketData
              }, function(response) {
                if (response && response.ok) {
                  hideLoadingSpinner(), chrome.storage.local.set({
                    [currentID + "classification"]: response.classification
                  }, function() {});
                  var labels = Array.from(document.querySelectorAll("label"));
                  let meta = [ "Service", "Ticket Type", "Request Type", "App/Hardware", "Assignment group" ];
                  var metaValues = {};
                  for (let i = 0; i < meta.length; i++) {
                    var label = labels.find(el => el.textContent.trim() === meta[i]);
                    label && (label = label.getAttribute("for"), label = document.getElementById(label)) && (label = label.value, 
                    metaValues[meta[i]] = label);
                  }
                  var referenceTicketText = Array.isArray(response.referenceTickets) && 0 < response.referenceTickets.length ? response.referenceTickets.map((reference, index) => {
                    var metadata = reference.metadata || {};
                    return `${index + 1}. ${metadata.ticket_id || "Chunk " + (reference.id || "N/A")} | Score: ${Number.isFinite(Number(reference.score)) ? Number(reference.score).toFixed(4) : "N/A"} | Service: ${metadata.service_type || "N/A"} | Ticket Type: ${metadata.ticket_type || "N/A"} | Request Type: ${metadata.request_type || "N/A"} | App/Hardware: ${metadata.app_hardware || "N/A"} | Assignment Group: ` + (metadata.assignment_group || "N/A");
                  }).join("\n") : "None";
                  tmpAlert("Current result: " + currentID + " | " + JSON.stringify(metaValues) + "\nClassification result: " + response.classification + "\nReference tickets:\n" + referenceTicketText, 6e4);
                } else hideLoadingSpinner(), tmpAlert(response && response.error ? response.error : "Classification failed.", 5e3);
              });
            } else if (value.startsWith("Upload to server")) if (-1 != currentURL.indexOf("incident.do.html")) tmpAlert("This feature only works on the real incident page.", 5e3); else {
              ...
              chrome.storage.local.get([ ticketDone ], function(result) {
                if (!chrome.runtime.lastError) {
                  ...
                  let meta = [ "Service", "Ticket Type", "Request Type", "App/Hardware", "Assignment group" ], metaValues = {};
                  for (let i = 0; i < meta.length; i++) {
                    var label = labels.find(el => el.textContent.trim() === meta[i]);
                    label && (label = label.getAttribute("for"), label = document.getElementById(label)) && (label = label.value, 
                    metaValues[meta[i]] = label);
                  }
                  var missingFields = meta.filter(field => !("Request Type" === field && "Request" !== metaValues["Ticket Type"] || metaValues[field] && "" !== metaValues[field].trim()));
                  if (0 < missingFields.length) {
                    const fieldsMsg = missingFields.join(", ");
                    tmpAlert(`The following fields are empty: ${fieldsMsg}. Please fill them before uploading.`, 5e3), 
                    void setTimeout(() => {
                      tmpAlert(`Missing fields: ${fieldsMsg}. Skipping upload and moving to next ticket.`, 5e3), 
                      processButton(!1, -1, [ "Delay w", "f" ]);
                    }, 5e3);
                  } else ticketData = {
                    ticket_id: currentID,
                    service_type: metaValues.Service,
                    ticket_type: metaValues["Ticket Type"],
                    request_type: metaValues["Request Type"],
                    app_hardware: metaValues["App/Hardware"],
                    assignment_group: metaValues["Assignment group"],
                    conversation: desc + "\n" + myPrompt,
                    request_type: metaValues["Request Type"]
                  }, tmpAlert("Uploading ticket in background...", 3e3), chrome.runtime.sendMessage({
                    type: "upload-conversation",
                    conversation: ticketData
                  }, function(response) {
                    response && response.ok ? (chrome.storage.local.set({
                      [ticketDone]: "true"
                    }, function() {}), tmpAlert("Server response: " + response.message, 3e3)) : (tmpAlert(response && response.error ? response.error : "Upload failed.", 5e3), 
                    response && response.error && response.error.includes("already uploaded earlier") && chrome.storage.local.set({
                      [ticketDone]: "true"
                    }, function() {})), setTimeout(() => {
                      processButton(!1, -1, [ "Delay w", "f" ]);
                    }, 2e3);
              ...
              var button;
              "" === userID || void 0 === userID ? (button = document.querySelector('[id="viewr.incident.caller_id"]')).disabled || (button.click(), 
              setTimeout(() => {
                null == (texbox = document.getElementById("sys_readonly.sys_user.user_name")) ? tmpAlert("Failed to get user ID. Please make sure the caller_id field is visible and try again.", 2e3) : (userID = texbox.value.split("@")[0].toLowerCase(), 
                button.click(), chrome.storage.local.set({
                  [curentID + "_userID"]: userID
                }, function() {}), chrome.storage.local.set({
                  askOODCommand: "quota for " + userID
                }, function() {}));
              }, 1e3)) : chrome.storage.local.set({
                askOODCommand: "quota for " + userID
              }, function() {});
            } else if (value.startsWith("Ask chatgpt to suggest")) if (-1 != currentURL.indexOf("incident.do.html")) tmpAlert("This feature only works on the real incident page.", 5e3); else if ("" == chatSessionURL) alert("Please set up the value for chatSessionURL in option page."); else {
              let desc = "User:\nShort description:\n" + val('textarea[name="incident.short_description"]') + "\nLong description:\n" + val('textarea[name="incident.description"]') + "\n", myPrompt1 = "";
              sendToChatSession(myPrompt1 = "" == myPrompt ? "Prompt: I (" + myFirstName + ") am a supporting staff. Here is my conversation with computer user.\nPlease suggest an answer for the user.  Please use the most recently attached ChatConext.date.json file as reference. Directly give the answer in plain text. Thanks.\n" + myFirstName + ": Welcome to visit us. What on your mind?\n\n" + desc + myPrompt : "Prompt: I (" + myFirstName + ") am a supporting staff. Here is my conversation with computer user.\n\nPlease suggest an answer for the user. Directly give the answer in plain text. Please use the most recently attached ChatConext.date.json file as reference. Thanks.\n" + desc + myPrompt);
            } else if (value.startsWith("Ask chatgpt to classify not in use")) if ("" == chatSessionURL) alert("Please set up the value for chatSessionURL in option page."); else {
              let desc = "User:\nShort description:\n" + val('textarea[name="incident.short_description"]') + "\nLong description:\n" + val('textarea[name="incident.description"]') + "\n";
              data = "Prompt: This is a request from computer user: \n\n" + desc + "\n\n" + value.split(":")[1] + "\n\nReturn your response in this exact format:\nClassification: [your classification]\n\nThanks.";
              sendToChatSession(data);
            } else if (value.startsWith("Ask chatgpt to classifyold")) if ("" == chatSessionURL) alert("Please set up the value for chatSessionURL in option page"); else {
              let desc = "User:\nShort description:\n" + val('textarea[name="incident.short_description"]') + "\nLong description:\n" + val('textarea[name="incident.description"]') + "\n", myPrompt1 = "";
              ...
              var repo, branchEl, chatContext = window.location.pathname.split("/");
              "pull" === chatContext[3] && (data = chatContext[1], repo = chatContext[2], 
              chatContext = chatContext[4], branchEl = document.querySelector(".head-ref")) && (branchEl = branchEl.textContent.trim(), 
              navigator.clipboard.writeText(`
                git clone https://github.com/${data}/${repo}.git
                cd ${repo}
                git fetch origin pull/${chatContext}/head:${branchEl}
              ...
              bestCategory;
            })(val('textarea[name="incident.short_description"]') + "\n" + val('textarea[name="incident.description"]')); else if (value.startsWith("Classify ticket0")) if ("" == chatSessionURL) alert("Please set up the value for chatSessionURL in option page"); else {
            ...
            } else if (value.startsWith("Test rag")) sendToChatSession(` You are a helpful assistant. Use the following context to answer the question.
            ...
            } else if (value.startsWith("Edit")) "" != currentText ? "" == chatSessionURL ? alert("Please set value for variable chatSessionURL in option page.") : sendToChatSession("Please edit: " + currentText) : tmpAlert("No message to edit.", 5e3); else if (value.startsWith("Summarize tickets0")) if ("" === myPrompt) tmpAlert("No conversaction is found"); else {
              let myPrompt1;
              if ("" == chatSessionURL) alert("Please set value for variable chatSessionURL in option page."); else {
                let desc = "User:\nShort description:\n" + val('textarea[name="incident.short_description"]') + "\nLong description:\n" + val('textarea[name="incident.description"]') + "\n";
                sendToChatSession(myPrompt1 = "Prompt: Please summerize the following conversation to a prompt context, extract the keywords in the questions and attach to the end of the paragraph. Please remove people's names: \n\n" + desc + myPrompt + "\n\nThanks.");
              }
              ...
              reload: "hi"
            }); else if (value.startsWith("Prompt")) if ("" != mostRecentMessage) tmpAlert("<p><strong>Sent chat message:</strong></p>" + mostRecentMessage, 5e3, !0), 
            chrome.runtime.sendMessage({
              chatInput: mostRecentMessage
            }), myPrompt += "\nUser: " + mostRecentMessage; else tmpAlert("<p><strong>No last message</strong></p>", 5e3, !0), 
            chrome.runtime.sendMessage({
  ...
  function removeHints() {
    let doc = document;
    for (var elements = (doc = newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body : doc).getElementsByClassName("addedhere"); 0 < elements.length; ) elements[0].parentNode.removeChild(elements[0]);
    remvoveHintWindow(), hintMsg = "";
  }
  function getCaretPosition(item) {
    let caretP = 0;
    var range, preRange, sel;
    return item && (item.isContentEditable || item.getAttribute && "true" === item.getAttribute("contenteditable")) ? (sel = window.getSelection()) && 0 < sel.rangeCount && ((preRange = (range = sel.getRangeAt(0)).cloneRange()).selectNodeContents(item), 
    preRange.setEnd(range.endContainer, range.endOffset), caretP = preRange.toString().length) : item.selectionStart || "0" == item.selectionStart ? caretP = item.selectionStart : document.selection && (item.focus(), 
  ...
  function hideRoot() {
    return (document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010")?.shadowRoot?.querySelector("iframe"))?.contentDocument ?? null;
  }
  function showAll() {
    if (isHiding = !1, newFormat) {
      var k, root = hideRoot();
      for (k of hideMap.keys()) {
        var el = root && qs(k, root);
        el && (el.style.display = "");
      }
    } else {
      for (let k of hideMap.keys()) showEls(k);
      var cs = qsa(".h-card.h-card_md.h-card_comments");
      if (null != cs && 0 < cs.length) for (var i = 0; i < cs.length; i++) cs[i].style.display = "";
    }
    window.scrollTo(0, 0);
  }
  function hideAll() {
    if (!urlMap.get("disableHiding") || !urlMap.get("disableHiding").endsWith("1")) if (shortDescriptionReadOnly(), 
    isHiding = !0, newFormat) {
      var k, root = hideRoot();
      for (k of hideMap.keys()) {
        var el = root && qs(k, root);
      ...
      for (let k of hideMap.keys()) hideEls(k);
      var cs = qsa(".h-card.h-card_md.h-card_comments");
      if (null != cs && 0 < cs.length) for (var i = 0; i < cs.length; i++) {
        var cType = cs[i].childNodes[1]?.innerText ?? "";
        -1 === cType.indexOf("Additional comments") && -1 === cType.indexOf("Work notes") && -1 === cType.indexOf("ServiceNow email") && -1 === cType.indexOf("Attachment") && (cs[i].style.display = "none");
      }
    }
  }
  function setupClickEventListerner(isIFrame) {
    var doc;
    0 < urlMap.get("submit").length && (doc = document, isIFrame && (doc = document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body), 
    isIFrame ? ((isIFrame = document.getElementById("gsft_main")?.contentDocument) ? qsa(urlMap.get("submit"), isIFrame) : []).forEach(submitBtn => {
      ...
      if ("unDoSendingButton" == e) {
        ...
        for (let j = 0; j < 10; j++) clearTimeout(timeOuts[j]);
        removeHints(), (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(urlMap.get("submit") + "_bottom") : qsa(urlMap.get("submit") + "_bottom")[0]).click(), 
  ...
  function submitButtonClicked() {
    if (null != lastText && 0 < lastText.length && null != lastTarget) {
      ...
      lastText = "";
    } else (newFormat ? document.querySelector("macroponent-f51912f4c700201072b211d4d8c26010").shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(urlMap.get("submit") + "_bottom") : qsa(urlMap.get("submit") + "_bottom"))[0].click(), 
    sending = !1;
  ...
  document.onmouseover = function(e) {
    ...
    let targetElement = qsa('textarea[name="incident.short_description"]');
    var rect;
    null != (targetElement = null != (targetElement = null != (targetElement = null != (targetElement = null != targetElement && 0 !== targetElement.length ? targetElement : qsa('input[name="incident.short_description"]')) && 0 !== targetElement.length ? targetElement : qsa('textarea[name="u_incident_task.short_description"]')) && 0 !== targetElement.length ? targetElement : qsa('textarea[name="sc_task.short_description"]')) && 0 !== targetElement.length ? targetElement : qsa("#form_main")) && 0 !== targetElement.length && (rect = targetElement[0].getBoundingClientRect(), 
    ...
    e = e.clientY - mousePosition.y, 20 < (distanceTraveled = Math.sqrt(rect * rect + e * e))) && (updateEditControlMode(!1), 
    -1 != currentURL.indexOf("chatgpt.com") ? (lastTarget.blur(), changeBackground("#FFFFCC")) : -1 != currentURL.indexOf("incident.do") && (lastTarget.blur(), 
  ...
  let currentBookmarkColor = "", currentStateColor = "";
  function ticketStateLabel(color) {
    var c = String(null == color ? "" : color).trim().toLowerCase();
    if (!c) return "";
    var key, label;
    for ([ key, label ] of [ [ "newTicketColor", "New" ], [ "assignedTicketColor", "Assigned" ], [ "holdTicketColor", "On hold" ], [ "closedTicketColor", "Closed" ], [ "inActionColor", "Working" ] ]) {
      var configured = String(urlMap.get(key) || "").trim().toLowerCase();
      if (configured && configured === c) return label;
    }
    return "white" === c || "#fff" === c || "#ffffff" === c ? "" : "Other";
  }
  function readableTextOn(color) {
    try {
      var r, g, b, probe = document.createElement("span"), rgb = (probe.style.color = color, 
      document.body.appendChild(probe), getComputedStyle(probe).color.match(/\d+/g));
      return probe.remove(), rgb ? ([ r, g, b ] = rgb.map(Number), 140 < (299 * r + 587 * g + 114 * b) / 1e3 ? "#111" : "#fff") : "#111";
    } catch (e) {
      return "#111";
    }
  }
  ...
  function statusPanel() {
    var panel = document.getElementById(STATUS_PANEL_ID);
    if (!panel) {
      if (!document.body) return null;
      (panel = document.createElement("div")).id = STATUS_PANEL_ID, panel.setAttribute("data-bw-ui", ""), 
      panel.style.cssText = [ "position:fixed", "left:8px", "top:8px", "z-index:2147483646", "min-width:120px", "max-width:200px", "padding:10px 12px", "border-radius:8px", "box-shadow:0 4px 16px rgba(0,0,0,.35)", "font-family:-apple-system,system-ui,Calibri,Arial,sans-serif", "text-align:center", "pointer-events:none" ].join(";"), 
  ...
  function statusLine(text, css) {
    ...
    return line.style.cssText = css, line.textContent = text, line;
  }
  function renderStatusPanel() {
    var label, text, background, panel = statusPanel();
    panel && (text = readableTextOn(background = (label = ticketStateLabel(currentStateColor)) ? currentStateColor : STATUS_DEFAULT_BG), 
    panel.style.setProperty("background-color", background, "important"), panel.style.setProperty("color", text, "important"), 
    panel.style.setProperty("--bw-status-bg", background), panel.textContent = "", 
    label && panel.appendChild(statusLine(label, "font-size:20px;font-weight:700;line-height:1.25")), 
    currentBookmarkColor && (text = statusLine("", "font-size:13px;font-weight:600;margin-top:4px"), 
    (background = document.createElement("span")).style.cssText = "display:inline-block;width:9px;height:9px;border-radius:50%;margin-right:5px;vertical-align:middle;border:1px solid rgba(0,0,0,.35);background-color:" + currentBookmarkColor, 
    text.append(background, document.createTextNode("Bookmarked")), panel.appendChild(text)), 
    panel.appendChild(statusLine(editingMode ? "Edit mode" : "Control mode", "font-size:12px;margin-top:4px;font-weight:" + (editingMode ? "700" : "600") + ";opacity:" + (editingMode ? "1" : ".75"))));
  }
  function showTicketState(color) {
    var label = ticketStateLabel(color);
    currentStateColor = label ? color : "", renderStatusPanel();
  }
  function showBookmarkTag(color) {
    color = String(null == color ? "" : color).trim();
    currentBookmarkColor = color && "white" !== color.toLowerCase() ? color : "", 
    renderStatusPanel();
  }
