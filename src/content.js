if (!window.__betterWebContentLoaded) {
  ...
  function processData(ob) {
    ...
    listPagePaths = splitWords(urlMap.get("listPagePaths")), (detailPagePaths = splitWords(urlMap.get("detailPagePaths"))).some(path => currentURL.includes(path))) {
      ...
      for (let i = 0; i < keys.length; i++) if (-1 == obj[keys[i]][0].indexOf("#")) {
        ...
        k.length < 3 && (/[^a-zA-Z0-9]/.test(k) || (helpMsg.set(k, "<p><strong>" + k + ": </strong>" + obj[keys[i]][0].split(" ").slice(1).join(" ") + "</p>"), 
      ...
      if (-1 != currentURL.indexOf("office.com/calendar")) (leftLineParts = new MutationObserver(function(mutations) {
        ...
        if (-1 != currentURL.indexOf("outlook.office.com")) window.onfocus = function() {
          chrome.storage.local.get([ "searchEmail" ], function(result) {
            ...
            document.querySelector('[data-testid="send-button"]'), $(".form-inline.navpage-global-search.ng-non-bindable").submit(), 
          ...
          document.addEventListener("click", e => {
            ...
            if ("rgba(0, 0, 0, 0)" === bgColor && ("DIV" === e.target.nodeName || "TD" === e.target.nodeName || -1 != currentURL.indexOf("testPage"))) {
              ...
              null != (targetElement = null != (targetElement = null != (targetElement = null != (targetElement = null != (targetElement = null != targetElement && 0 !== targetElement.length ? targetElement : $('input[name="incident.short_description"]')) && 0 !== targetElement.length ? targetElement : $('textarea[name="u_incident_task.short_description"]')) && 0 !== targetElement.length ? targetElement : $('textarea[name="sc_task.short_description"]')) && 0 !== targetElement.length ? targetElement : $('textarea[name="pm_project_task.description"]')) && 0 !== targetElement.length ? targetElement : $("#form_main")) && void 0 !== targetElement && (bgColor = targetElement[0].getBoundingClientRect(), 
            ...
            hintText.clear())) : lastTarget = e.target, isSelfClick || ("TEXTAREA" === e.target.nodeName && "incident.description" != e.target.id ? (updateEditControlMode(!0), 
            selectedText = e.target.value.substring(e.target.selectionStart, e.target.selectionEnd)) : -1 != currentURL.indexOf("incident_do") && "" == $('[name*="assigned_to"], [id*="assigned_to"]').val() || -1 != currentURL.indexOf("incident_task") && "" === $('input[name="sys_display.u_incident_task.assigned_to"]').val() || -1 != currentURL.indexOf("sc_task") && "" === $('input[name="sys_display.sc_task.assigned_to"]').val() ? (changeBackground(urlMap.get("newTicketColor")), 
  ...
  function shortDescriptionEditable() {
    try {
      "" != $('input[name="incident.short_description"]').val() && $('input[name="incident.short_description"]').removeAttr("readonly"), 
      "" != $('textarea[name="incident.short_description"]').val() && $('textarea[name="incident.short_description"]').removeAttr("readonly"), 
      "" != $('textarea[name="incident.description"]').val() && $('textarea[name="incident.description"]').removeAttr("readonly"), 
      "" != $('textarea[name="u_incident_task.short_description"]').val() && $('textarea[name="u_incident_task.short_description"]').removeAttr("readonly"), 
      "" != $('textarea[name="sc_task.short_description"]').val() && $('textarea[name="sc_task.short_description"]').removeAttr("readonly");
    } catch (e) {}
  }
  function makeDescriptionReadOnly() {
    ...
    for (key in chrome.storage.local.get([ currentID + "color" ], function(result) {
      ...
      assignedTo = $('[name*="assigned_to"], [id*="assigned_to"]').val(), "green" === result && assignedTo && -1 != assignedTo.indexOf(myFirstName) || showBookmarkTag(result));
  ...
  async function processIFrame(iframe) {
    ...
    if (null != iframe || rootDoc === document) {
      ...
      for (let i = 0; i < ls.length; i++) {
        ...
        if (-1 === $(datePart).text().toLowerCase().indexOf(myID.toLowerCase())) {
        ...
        void 0 === seconds || "green" === seconds && -1 != $(timePart).text().indexOf(myFirstName) || (ls[i].style.backgroundColor = seconds);
      }
...
`, el.textContent = msg;
  ...
  $(document).ready(function() {
    ...
    loadCon();
    {
      var callback = function() {
        ...
        null != $("macroponent-f51912f4c700201072b211d4d8c26010")[0] && alert("Please disable 'Always show top navigation' in 'Settings' > 'Display' or enable 'Turn off Next Experience' in 'Settings' > 'User experience' > 'Turn off Next Experience'"), 
        chrome.storage.local.get([ "delayKeyPressAfterReload" ], function(result) {
          if (!chrome.runtime.lastError) {
          ...
          cmdProcessTimeout = setTimeout(() => {
            ...
            showAll()); else if (value.startsWith("Click")) {
              ...
              const clickExistingControl = function() {
                ...
                newFormat ? (macroponent = (macroponent = (macroponent = $("macroponent-f51912f4c700201072b211d4d8c26010")[0]) && macroponent.shadowRoot ? macroponent.shadowRoot.querySelector("iframe") : null) && macroponent.contentWindow && macroponent.contentWindow.document.body ? macroponent.contentWindow.document.body.querySelector(selector) : null) && (macroponent.click(), 
                setTimeout(() => {
                  processIFrame(null);
                }, 4e3)) : $(selector).eq(0).click();
              };
            ...
            chrome.storage.local.set({
              ...
              tmpAlert("Tickets will be stop downloadeding.", 4e3);
            }); else if (value.startsWith("Copy:")) if (-1 != value.indexOf("userID")) "" === userID ? (button = document.querySelector('[id="viewr.incident.caller_id"]')).disabled || (button.click(), 
            setTimeout(() => {
              var v;
            ...
            tmpAlert("Copied: " + chatContext)); else {
              let v = value.slice(5).replace("ticketID", currentID).replace("jobID", jobID).replace("link", ticketLink).replace("shortDescription", $('textarea[name="incident.short_description"]').val());
              ...
              var button;
              "" === userID ? (button = document.querySelector('[id="viewr.incident.caller_id"]')).disabled || (button.click(), 
              setTimeout(() => {
                null == (texbox = document.getElementById("sys_readonly.sys_user.user_name")) ? tmpAlert("Failed to get user ID. Please make sure the caller_id field is visible and try again.", 2e3) : (userID = texbox.value.split("@")[0].toLowerCase(), 
  ...
  function hideAll() {
    ...
    isHiding = !0, newFormat) for (var k of hideMap.keys()) $("macroponent-f51912f4c700201072b211d4d8c26010")[0].shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(k).style.display = "none"; else {
