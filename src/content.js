if (!window.__betterWebContentLoaded) {
  ...
  var delayTimeout, backgroundChangeable = !1, shortDescrtionYValue = 0, pageOpenTime = 0, pageCloseTime = 0, caretPosition = 0, searchTerms = [], allTicketIDs = [], domain = "", editingMode = !1;
  ...
  function processButton(newTabOpened, labelIndex, labelValuesPairs) {
    if (tmpAlert(labelIndex + " : " + labelValuesPairs[labelIndex], 1e3), newTabOpened) chrome.storage.local.get([ "newTabOpened" ], function(result) {
      null != result.newTabOpened && "no" == result.newTabOpened && (newTabOpened = !1);
    }); else {
      if (++labelIndex === labelValuesPairs.length) return;
      if (1 == labelValuesPairs[labelIndex].length) document.dispatchEvent(new KeyboardEvent("keyup", {
        ...
        editableDiv && (targetID1 = className), ctrlDown || !outsideKeyPress && "readonly" != readonly) {
          ...
          cmdProcessTimeout = setTimeout(() => {
            ...
            chrome.storage.local.remove(currentID, function() {}), tmpAlert("Removed bookmark for this ticket!"); else if (value.startsWith("Bookmark")) {
              ...
              }, function() {}), tmpAlert("Bookmark color is set to " + tenure), 
              setTimeout(() => {
                chrome.storage.local.get([ currentID + "color" ], function(result) {
            ...
            chrome.storage.local.set({
              ...
              setTimeout(() => {
                ...
                tmpAlert("Copied: " + v));
              }, 1e3)) : (copyToClipboard(tenure = value.slice(5).replace("ticketID", currentID).replace("jobID", jobID).replace("link", ticketLink).replace("shortDescription", $('textarea[name="incident.short_description"]').val())), 
  ...
  function hideAll() {
    if (!urlMap.get("disableHiding") || !urlMap.get("disableHiding").endsWith("1")) if (isHiding = !0, 
    newFormat) for (var k of hideMap.keys()) $("macroponent-f51912f4c700201072b211d4d8c26010")[0].shadowRoot.querySelector("iframe").contentWindow.document.body.querySelector(k).style.display = "none"; else {
  ...
  $(document).ready(function() {
    setupEditControlModeStatus(), pageOpenTime = new Date().getTime(), currentURL = window.location.href, 
    domain = currentURL.startsWith("file://") ? "file://Users/ld32/projects/betterWebPrivate" : -1 != currentURL.indexOf("testPage") ? "https://ld32.github.io/betterWeb" : "https://" + currentURL.match(/https:\/\/([^\/]+)/)[1], 
          ...
          classifyButton.style.zIndex = "10000", classifyButton.onclick = function() {
            ...
            !function processTicket(i) {
              if (i >= allTicketIDs.length) if (0 < classifyTicketList.length) {
                ...
                -1 != currentURL.indexOf("testPage") && (url = domain + "/testPage/incident.do.html?sys_id=" + classifyTicketList[0].split(":")[1]), 
                chrome.runtime.sendMessage({
                  greeting: "Open " + url
                }, function(response) {});
